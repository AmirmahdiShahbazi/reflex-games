...
const BEST_SCORE_KEY = 'dodge-best-score'
...
function DodgeGame() {
  ...
  const targetPlayerRef = useRef<Player>({ x: 50, y: 50 })
  ...
  const enemySpeedRef = useRef(INITIAL_SPEED)
  ...
  useEffect(() => {
    ...
    nearMissSound.current = new Audio('/sounds/hit.wav')
    ...
    return () => {
      ...
      nearMissSound.current = null
    }
  }, [])

  const playGameOverSound = () => {
    const sound = gameOverSound.current
    if (!sound) return
    sound.currentTime = 0
    sound.play().catch(() => {})
  }
  ...
  const playNearMissSound = () => {
    const sound = nearMissSound.current
    if (!sound) return
    sound.currentTime = 0
    sound.play().catch(() => {})
  }
    ...
    async (finalScore: number) => {
      try {
        ...
        if (typeof data.record === 'number') {
          ...
          bestScoreRef.current = data.record
  ...
  const getElapsedSeconds = () => {
    return (performance.now() - gameStartTimeRef.current) / 1000
  }

  const getDesiredEnemyCount = () => {
    const seconds = getElapsedSeconds()
    return Math.min(MAX_ENEMIES, INITIAL_ENEMIES + Math.floor(seconds / 1.7))
  }
  ...
  const getCurrentSpeed = () => {
    ...
    return Math.min(
      MAX_SPEED,
      INITIAL_SPEED + seconds * 9 + Math.pow(seconds, 1.2) * 2,
    )
  }
  ...
  const createEnemy = (): Enemy => {
    ...
    const seconds = getElapsedSeconds()
    const difficulty = Math.min(1, seconds / 45)
    const random = Math.random()

    let type: EnemyType = 'normal'

    if (seconds > 4 && random < 0.2 + difficulty * 0.08) {
      type = 'fast'
    }
    if (seconds > 9 && random >= 0.72 && random < 0.82) {
      type = 'straight'
    }
    if (seconds > 14 && random >= 0.82 && random < 0.92) {
      type = 'heavy'
    }
    if (seconds > 20 && random >= 0.92) {
      type = 'zigzag'
    }

    let size =
      MIN_ENEMY_SIZE + Math.random() * (MAX_ENEMY_SIZE - MIN_ENEMY_SIZE)
    ...
    let speed = enemySpeedRef.current

    if (type === 'fast') speed *= 1.55
    if (type === 'straight') speed *= 1.2
    if (type === 'heavy') speed *= 0.68
    if (type === 'zigzag') speed *= 1.1
    ...
    return {
      ...
      zigzagTimer: Math.random() * ZIGZAG_INTERVAL,
  ...
  const startGame = () => {
    ...
    targetPlayerRef.current = initialPlayer
    enemiesRef.current = []

    scoreRef.current = 0
    nearMissesRef.current = 0
    comboRef.current = 0
    enemySpeedRef.current = INITIAL_SPEED
  ...
  const movePlayer = (clientX: number, clientY: number) => {
    const arena = gameAreaRef.current
    if (!arena) return
  ...
  const isPlayerHit = (
    ...
    const dx = ((enemy.x - currentPlayer.x) * width) / 100
    const dy = ((enemy.y - currentPlayer.y) * height) / 100
    const distance = Math.sqrt(dx * dx + dy * dy)
    const collisionRadius = PLAYER_SIZE / 2 + enemy.size / 2
  ...
  const isNearMiss = (
    ...
    if (enemy.nearMissTriggered) return false
    ...
    return (
      ...
      distance < collisionRadius + NEAR_MISS_DISTANCE
    )
  }
  ...
  const resolveBallCollisions = (
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        a.x -= (nx * separation * 100) / width
        a.y -= (ny * separation * 100) / height
        b.x += (nx * separation * 100) / width
        b.y += (ny * separation * 100) / height
        ...
        if (relativeVelocity < 0) {
          const impulse =
            (-(1 + BALL_BOUNCE) * relativeVelocity) / 2
          a.vx -= impulse * nx
          a.vy -= impulse * ny
          b.vx += impulse * nx
          b.vy += impulse * ny
  ...
  useEffect(() => {
    ...
    const spawn = () => {
      ...
      const desired = getDesiredEnemyCount()
      const current = enemiesRef.current.length

      if (current < desired && current < MAX_ENEMIES) {
        enemiesRef.current.push(createEnemy())
        ...
        if (
          seconds > 18 &&
          Math.random() < 0.25 &&
          enemiesRef.current.length < desired
        ) {
          enemiesRef.current.push(createEnemy())
        }
      }
    ...
    spawnTimerRef.current = window.setInterval(spawn, SPAWN_CHECK_INTERVAL)
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      const seconds = (currentTime - gameStartTimeRef.current) / 1000
      enemySpeedRef.current = getCurrentSpeed()

      const currentPlayer = playerRef.current
      const targetPlayer = targetPlayerRef.current
      const playerLerp = Math.min(1, deltaTime * PLAYER_LERP)

      currentPlayer.x +=
        (targetPlayer.x - currentPlayer.x) * playerLerp
      currentPlayer.y +=
        (targetPlayer.y - currentPlayer.y) * playerLerp

      setSurvivalTime(Number(seconds.toFixed(1)))

      if (currentTime - lastScoreTimeRef.current >= SCORE_INTERVAL) {
        const points = Math.floor(
          (currentTime - lastScoreTimeRef.current) / 1000,
        )
      ...
      if (
        comboRef.current > 0 &&
        currentTime - lastNearMissTimeRef.current > 2600
      ) {
        comboRef.current = 0
        setCombo(0)
      }
      ...
      const updatedEnemies = enemiesRef.current.map((enemy) => {
        ...
        if (enemy.type === 'normal') {
          ...
          const steering = Math.min(1, deltaTime * 2.8)
          enemy.vx += (targetVX - enemy.vx) * steering
          enemy.vy += (targetVY - enemy.vy) * steering
        } else if (enemy.type === 'fast') {
          const speed = enemySpeedRef.current * 1.55
          const targetVX = (dx / distance) * speed
          const targetVY = (dy / distance) * speed
          const steering = Math.min(1, deltaTime * 4)
          enemy.vx += (targetVX - enemy.vx) * steering
          enemy.vy += (targetVY - enemy.vy) * steering
        } else if (enemy.type === 'straight') {
          const speed =
            Math.sqrt(enemy.vx * enemy.vx + enemy.vy * enemy.vy) || 1
          const targetSpeed = enemySpeedRef.current * 1.2
          enemy.vx = (enemy.vx / speed) * targetSpeed
          enemy.vy = (enemy.vy / speed) * targetSpeed
        } else if (enemy.type === 'heavy') {
          const speed = enemySpeedRef.current * 0.68
          const targetVX = (dx / distance) * speed
          const targetVY = (dy / distance) * speed
          const steering = Math.min(1, deltaTime * 1.8)
          enemy.vx += (targetVX - enemy.vx) * steering
          enemy.vy += (targetVY - enemy.vy) * steering
          ...
          if (enemy.zigzagTimer <= 0) {
            enemy.zigzagTimer =
              ZIGZAG_INTERVAL * (0.7 + Math.random() * 0.5)
            enemy.zigzagDirection *= -1
          }
          const targetAngle = Math.atan2(dy, dx)
          const angle =
            targetAngle + enemy.zigzagDirection * 0.8
          const speed = enemySpeedRef.current * 1.1
          const targetVX = Math.cos(angle) * speed
          const targetVY = Math.sin(angle) * speed
          const steering = Math.min(1, deltaTime * 3)
          enemy.vx += (targetVX - enemy.vx) * steering
          enemy.vy += (targetVY - enemy.vy) * steering
        }
        ...
        const velocity =
          Math.sqrt(enemy.vx * enemy.vx + enemy.vy * enemy.vy) || 1
        const maximum =
          enemySpeedRef.current * (enemy.type === 'fast' ? 1.7 : 1.35)
        ...
        enemy.x += (enemy.vx * deltaTime * 100) / width
        enemy.y += (enemy.vy * deltaTime * 100) / height

        const radiusX = (enemy.size * 0.5 * 100) / width
        const radiusY = (enemy.size * 0.5 * 100) / height

        if (enemy.x < radiusX) {
          enemy.x = radiusX
          enemy.vx = Math.abs(enemy.vx)
        }
        if (enemy.x > 100 - radiusX) {
          enemy.x = 100 - radiusX
          enemy.vx = -Math.abs(enemy.vx)
        }
        if (enemy.y < radiusY) {
      ...
      resolveBallCollisions(updatedEnemies, width, height)

      for (const enemy of updatedEnemies) {
        if (isNearMiss(enemy, currentPlayer, width, height)) {
          enemy.nearMissTriggered = true
          nearMissesRef.current += 1
          comboRef.current += 1

          const bonus = 3 + Math.min(comboRef.current, 12)
          scoreRef.current += bonus

          lastNearMissTimeRef.current = currentTime
          setNearMisses(nearMissesRef.current)
      ...
      const collision = updatedEnemies.some((enemy) =>
        isPlayerHit(enemy, currentPlayer, width, height),
      )
  ...
  }, [gameStarted, gameOver, submitScore])
      ...
      <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-4 py-6 sm:px-5 sm:py-8">
        ...
        <header className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black sm:text-2xl">فرار کن</h1>
              <p className="text-xs text-zinc-500">هر ثانیه سخت‌تر میشه</p>
            </div>
          </div>
          ...
          <section className="flex flex-1 items-center justify-center">
            <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 text-center shadow-2xl sm:p-9">
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-white/10">
                <Shield size={40} strokeWidth={1.7} />
              </div>

              <h2 className="text-3xl font-black">فرار کن</h2>

              <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-zinc-400">
                توپ‌ها از همه طرف وارد میشن، به هم برخورد می‌کنن، از دیوار
                برمی‌گردن و هر لحظه سریع‌تر میشن. فقط جاخالی بده و زنده بمون.
              </p>

              <div className="mt-7 grid grid-cols-2 gap-2">
                <div className="rounded-2xl bg-white/5 p-3">
                  <p className="text-xs text-zinc-500">سختی</p>
                  <p className="mt-1 font-bold">بی‌رحمانه</p>
                </div>

                <div className="rounded-2xl bg-white/5 p-3">
                  <p className="text-xs text-zinc-500">دشمن</p>
                  <p className="mt-1 font-bold">تا ۳۰ توپ</p>
                </div>
              </div>
        ...
        {isPlaying && (
          <section className="flex flex-1 flex-col">
            ...
            <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-center sm:p-4">
                <p className="text-xs text-zinc-500">زمان بقا</p>
                <p className="mt-1 text-xl font-black sm:text-2xl">
                  {survivalTime.toFixed(1)} ثانیه
                </p>
              </div>

              <div className="hidden rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-center sm:block sm:p-4">
                <p className="text-xs text-zinc-500">فرار نزدیک</p>
                <p className="mt-1 text-xl font-black sm:text-2xl">
                  {nearMisses}
                </p>
              </div>
            ...
            >
              ...
              <div className="pointer-events-none absolute bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/5 bg-black/40 px-4 py-2 text-xs text-zinc-400 backdrop-blur-sm">
                نشانگر را حرکت بده و زنده بمون
              </div>
            </div>
          </section>
        )}
        ...
        {gameOver && (
          <section className="flex flex-1 items-center justify-center">
            <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 text-center shadow-2xl sm:p-8">
              ...
              <p className="text-xs text-zinc-500">گیر افتادی!</p>

              <h2 className="mt-1 text-5xl font-black">{score}</h2>
              <p className="mt-1 text-xs text-zinc-400">امتیاز نهایی</p>

              {/* Survival stats */}
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5">
                  <span className="text-zinc-500">زمان بقا:</span>{' '}
                  <span className="font-bold text-white">
                    {survivalTime.toFixed(1)}s
                  </span>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5">
                  <span className="text-zinc-500">فرارهای نزدیک:</span>{' '}
                  <span className="font-bold text-white">{nearMisses}</span>
                </div>
              </div>
